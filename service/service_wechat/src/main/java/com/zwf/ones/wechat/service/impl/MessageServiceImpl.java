...
import com.zwf.ones.client.course.CourseFeignClient;
import com.zwf.ones.model.vod.Course;
import com.zwf.ones.wechat.service.MessageService;
...
public class MessageServiceImpl implements MessageService {
        ...
        try {
            ...
            switch(msgType){
                case "text" ://普通文本类型，输入关键字java
                    content = this.search(param);//调用下面的search方法
                    break;
                case "event" ://关注  取消关注   点击关于我们
                    String event = param.get("Event");
                    String eventKey = param.get("EventKey");
                    if("subscribe".equals(event)) {//关注公众号
                        content = this.subscribe(param);//调用subscribe方法
                    } else if("unsubscribe".equals(event)) {//取消关注公众号
                        content = this.unsubscribe(param);//调用unsubscribe方法
                    } else if("CLICK".equals(event) && "aboutUs".equals(eventKey)){//菜单里面设置的菜单回复消息，没有该菜单可以不写
    ...
    public void pushPayMessage(long orderId) {
        String openid = "oFGYz6anAhebPc8l3SQWM4N20Cnc";
        WxMpTemplateMessage templateMessage = WxMpTemplateMessage.builder()
                .toUser(openid)//要推送的用户openid
                .templateId("WRPtjDKjWBKEV-dVNBzg2CKRMPaVUJLn6zy3P0YEJgw")//模板id
                .url("http://ones.free.idcfengye.com/#/pay/"+orderId)//点击模板消息要访问的网址
                .build();
    ...
    private String search(Map<String, String> param) {
        ...
        //远程调用course模块，注入private CourseFeignClient courseFeignClient
